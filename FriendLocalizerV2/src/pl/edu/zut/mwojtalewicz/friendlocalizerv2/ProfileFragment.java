...
public class ProfileFragment extends Fragment implements MyLocationIntrface {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		ScrollView mScrollView = (ScrollView) inflater.inflate(
		...
		tvHelloPerson.setText(userDetails.get("name") + " "
	...
	public void onAttach(Activity activity) {
		...
		if (isInternetPresent == true) {
			...
			if (json == null) {
				...
				// not.setNewAlertDialog("Błąd",
				// "Wystąpił problem przy przesyłaniu danych. Spróbuj ponownie później.",
				// getActivity());
			} else {
				try {
					if (json.get(Constans.KEY_SUCCESS) != null) {
						...
						if (Integer.parseInt(res) == 1) {
							Intent tokenIntent = new Intent(getActivity(),
									NewInviteToFriends.class);
							Notification not = new Notification(tokenIntent);
							not.displayOwnNotification(getActivity(),
									"Nowe zaproszenie!",
									"Zostałeś zaproszony do znajomych...",
									json.toString());
						} else {
				...
				if (json2.get(Constans.KEY_SUCCESS) != null) {
					...
					if (Integer.parseInt(res) == 1) {
						...
						Intent tokenIntent = new Intent(getActivity(),
								AcceptReplay.class);
						Notification not = new Notification(tokenIntent);
						not.displayOwnNotification(getActivity(), "Akceptacje",
								"Zostały zaakceptowane zaproszenia...",
								json2.toString());
					}
				}
			} catch (Exception e) {
			...
			setInternetConnectionAlertDialog("FriendLocalizer",
					"Brak aktywnego połączenia z internetem. Włączyć?",
					getActivity());
		}
	}

	private void setInternetConnectionAlertDialog(String title, String msg,
			Context context) {
		AlertDialog alertDialog = new AlertDialog.Builder(context).create();
		alertDialog.setTitle(title);
		alertDialog.setMessage(msg);
		alertDialog.setIcon(R.drawable.ic_launcher);
		alertDialog.setButton(DialogInterface.BUTTON_POSITIVE, "Tak",
		...
		alertDialog.setButton(DialogInterface.BUTTON_NEGATIVE, "Nie",
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						getActivity().finish();
	...
	public void sendGpsPosition(Double longitude, Double latitude) {
		...
		if (isInternetPresent) {
			...
			String uniqueID = userDetails.get("uid");
			if (uniqueID != null) {
				JSONObject json = usr.sendUserGpsLocation(uniqueID, ""
						+ longitude, "" + latitude);
